...
def move_matching_files(image_dir, txt_dir, dest_dir):
    ...
    for name in common_names:
        # 이미지 파일 경로 설정
        img_file_path = os.path.join(image_dir, f"{name}.jpg")
        if not os.path.exists(img_file_path):
            img_file_path = os.path.join(image_dir, f"{name}.jpeg")
        if not os.path.exists(img_file_path):
            img_file_path = os.path.join(image_dir, f"{name}.png")
        if not os.path.exists(img_file_path):
            img_file_path = os.path.join(image_dir, f"{name}.gif")
        if not os.path.exists(img_file_path):
            img_file_path = os.path.join(image_dir, f"{name}.bmp")
        ...
        if os.path.exists(img_file_path):
            ...
            print(f"Moved image file: {os.path.basename(img_file_path)}")
            moved_files_count += 1

        # 텍스트 파일 이동
        if os.path.exists(txt_file_path):
            shutil.move(txt_file_path, dest_txt_file_path)
            print(f"Moved text file: {os.path.basename(txt_file_path)}")
            moved_files_count += 1
...
# 원본 폴더와 대상 폴더 경로 설정
image_directory = r'C:\Users\AI Software\Desktop\BAC_HIEN_CONSTRUCTION_SAFETY_2024.v8i.yolov8\valid\a'
text_directory = r'C:\Users\AI Software\Desktop\BAC_HIEN_CONSTRUCTION_SAFETY_2024.v8i.yolov8\valid\labels'
destination_directory = r'C:\Users\AI Software\Desktop\BAC_HIEN_CONSTRUCTION_SAFETY_2024.v8i.yolov8\valid\al'
